...
/// Creates a ScatterPlotChart widget based on the JSON structure.
class ScatterPlotChartBuilder extends JsonWidgetBuilder {
  ScatterPlotChartBuilder({
  ...
  static const type = 'scatter_plot_chart';
  ...
  final common.PointRendererConfig<num>? defaultRenderer;
  ...
  /// }
  /// ```
  static ScatterPlotChartBuilder fromDynamic(
    dynamic map, {
    JsonWidgetRegistry? registry,
  }) {
    if (map == null) {
      throw Exception('[ScatterPlotChartBuilder]: map is null');
    }

    return ScatterPlotChartBuilder(
      animate:
      ...
      defaultRenderer: JsonChartsDecoder.decodePointRendererConfig<num>(
        map['defaultRenderer'],
        validate: false,
      ),
  ...
  Widget buildCustom({
    ...
    assert(
      ...
      '[ScatterPlotChartBuilder] does not support children.',
    );
